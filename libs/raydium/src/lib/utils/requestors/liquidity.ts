...
const _MARKET_STATE_LAYOUT_V2 = MARKET_STATE_LAYOUT_V2;
...
export const requestLiquidityInfos = async (
  ...
  marketAll.forEach((item) => {
    marketToLayout[item.publicKey.toString()] = _MARKET_STATE_LAYOUT_V2.decode(
  ...
  const liquidityPools = {} as ILiquidityPools;
  ...
  LIQUIDITY_POOLS.forEach((pool) => {
    ...
    const poolInfo: LiquidityPoolInfo = cloneDeep(pool);
